...
// HelidoHandler injects FLUENTD sidecar container for generic k8s Deployment
type HelidoHandler struct {
	client.Client
	Log logr.Logger
}

// Apply applies a logging scope to a k8s Deployment
func (h *HelidoHandler) Apply(ctx context.Context, workload vzapi.QualifiedResourceRelation, scope *vzapi.LoggingScope) error {
...
// Remove removes a logging scope from a k8s Deployment
func (h *HelidoHandler) Remove(ctx context.Context, workload vzapi.QualifiedResourceRelation, scope *vzapi.LoggingScope) (bool, error) {
	deploy, err := h.getDeployment(ctx, workload, scope)
	if err != nil {
		h.Log.Error(err, "Failed to fetch Deployment", "Deployment", workload.Name)
...
// Remove removes a logging scope from a k8s Deployment
func (h *HelidoHandler) getDeployment(ctx context.Context, workload vzapi.QualifiedResourceRelation, scope *vzapi.LoggingScope) (*kapps.Deployment, error) {
...
func (h *HelidoHandler) ensureFluentdConfigMap(ctx context.Context, namespace, appName string) error {
...
func (h *HelidoHandler) deleteFluentdConfigMap(ctx context.Context, namespace, appName string) error {
	name := fluentdConfigMapName(appName)
	configMap := &kcore.ConfigMap{}
	err := h.Get(ctx, objKey(namespace, name), configMap)
...
func (h *HelidoHandler) ensureEsSecret(ctx context.Context, namespace, name string) error {
	secret := &kcore.Secret{}
